...
test("parseEchoTag: extracts quoted args plus the raw final argument", () => {
  const out = parseEchoTag(['[out] ECHO: "@info", "Rim height", "mm", 0.48'], "@info", 2);
  assert.deepEqual(out, [["Rim height", "mm", "0.48"]]);
});

test("parseEchoTag: matches ECHO on stderr too (OpenSCAD-WASM routes ECHO to [err])", () => {
  const out = parseEchoTag(['[err] ECHO: "@review", "label", "GATE 12"'], "@review", 1);
  assert.deepEqual(out, [["label", '"GATE 12"']]);
});
