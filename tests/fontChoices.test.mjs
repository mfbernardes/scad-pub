...
test("fontValueLabel is the friendly face name, never the Fontconfig string", () => {
  assert.equal(fontValueLabel("Nimbus Sans Narrow:style=Regular"), "Nimbus Sans Narrow");
...
test("an installed face named by an enum choice reuses the choice's exact value", () => {
  ...
  assert.deepEqual(
    ...
    [["Nimbus Sans Narrow:style=Regular", "Nimbus Sans Narrow", false]]
  );
});
...
test("a selected missing enum choice is listed once, under the stored value", () => {
  const param = enumParam(["Nimbus Sans Narrow:style=Regular"]);
  const { missing } = buildFontChoices(param, "Nimbus Sans Narrow:style=Regular", FONTS);
  assert.equal(missing.length, 1);
  assert.equal(missing[0].value, "Nimbus Sans Narrow:style=Regular");
});
