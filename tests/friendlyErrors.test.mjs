...
// RenderResult to the friendly headline/body/technical-details copy shown by
// FriendlyError.tsx (the Notices tab card) and used for the render-failed
// toast title (useRenderPipeline.ts). See src/lib/diagnostics.ts for the
// ASSERT_RE/WARNING_RE this reuses rather than duplicates.
...
test("a fatal bootstrap failure wins priority over everything else", () => {
  ...
  assert.equal(info.title, t("failure.engineFailed"));
...
test("a failed assert with an authored message: title + verbatim, unquoted body", () => {
  ...
  const info = friendlyRenderError(result);
  assert.equal(info.title, t("failure.title"));
  assert.equal(
    info.body,
...
test("any other nonzero exit (no assert, not fatal): generic toast-matching title, null body", () => {
  ...
  const info = friendlyRenderError(result);
  assert.equal(info.title, t("toast.renderFailed"));
  assert.equal(info.body, null);
});
