...
import { parsePresetCardName } from "../src/lib/presetCard.ts";

test("a trailing parenthetical becomes the badge", () => {
  assert.deepEqual(parsePresetCardName("Large tag (Metric)"), {
    title: "Large tag",
    badge: "Metric",
  });
});
...
test("a leading 'Category | ' prefix becomes the overline, alongside a badge", () => {
  assert.deepEqual(parsePresetCardName("Signs | Door plate (Imperial)"), {
    overline: "Signs",
    title: "Door plate",
    badge: "Imperial",
  });
...
test("nested/multiple parens: only the trailing one splits off", () => {
  assert.deepEqual(parsePresetCardName("Setting (default) (Metric)"), {
...
test("degrades gracefully when the overline/title split would be empty", () => {
  ...
  // empty label.
  assert.deepEqual(parsePresetCardName("Category | (Metric)"), {
    title: "Category |",
    badge: "Metric",
  });
});
