...
function Tooltip({ ...props }: React.ComponentProps<typeof TooltipPrimitive.Root>) {
  // Self-contained: wraps every `Tooltip` in its own `TooltipProvider`
  // rather than requiring one mounted once near the app root. Radix's
  // `Tooltip` root throws ("`Tooltip` must be used within `TooltipProvider`")
  // without a provider ancestor — this repo has none mounted yet (this is
  // the first `Tooltip` consumer, `ViewerHUD.tsx`), matching the current
  // shadcn/ui scaffold convention of baking the provider in here so callers
