...
/* The help sections' typography, applied to the scrolling body wrapper (the
   ...
   Round-2 review fix (Help modal widened, see the `size="wide"` Modal call
   below): a wider dialog gives the tab strip and overall chrome more room to
   breathe, per the reviewer's "wider" ask, but letting prose stretch
   edge-to-edge at 760px would push lines well past a comfortable reading
   measure (~90+ characters). Rather than inventing a bespoke in-between
   Modal size just for this, the actual text content caps itself at a classic
   ~68-character reading measure (`max-w-[42rem]`, independent of the
   dialog's own width) — the dialog is wider, the type block that visitors
   actually read is not. */
const HELP_BODY = cn(
  MODAL_BODY,
  "help-body [&_section]:my-[0.9rem] [&_h3]:mb-1 [&_h3]:text-[0.95rem] [&_h3]:text-brand",
  "[&_p]:m-0 [&_p]:max-w-[42rem] [&_p]:text-[0.88rem] [&_p]:leading-[1.5] [&_p]:text-foreground",
  "[&_ul]:mt-[0.35rem] [&_ul]:max-w-[42rem] [&_ul]:pl-[1.1rem] [&_ul]:text-[0.88rem] [&_ul]:leading-[1.5] [&_ul]:text-foreground [&_li]:my-[0.2rem]"
);
...
export function HelpModal({
  ...
  return (
    // Round-2 review fix: widened from "default" (680px) to "wide" (760px,
    // shared with DesignPickerDialog) — the reviewer's own explicit "wider"
    // ask. See HELP_BODY's own doc above for why the wider dialog doesn't
    // just let prose lines stretch to match.
    <Modal title={content.title ?? t("help.defaultTitle")} label={t("help.label")} onClose={onClose} size="wide">
      {content.intro && (
        isMobile ? (
