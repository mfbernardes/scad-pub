...
import type { Insets, FitFraction } from "./framing";
...
interface Props {
  ...
  /** Whether the viewer's reference grid is drawn (config ui.grid / the HUD's
   *  grid toggle — see src/lib/viewerPrefs.ts). */
  showGrid: boolean;
  view: ViewName;
  /** Pixel margins excluded from the camera fit — the HUD strip, the export
   *  dock, a small top banner allowance — so the model frames against the
   *  actual unobscured viewport (see framing.ts's computeViewerInsets and
   *  AppShell's own insets useMemo). Passed straight through to Viewer. */
  insets: Insets;
  /** Round-5 Wave 2 (item 7): per-mount fill-fraction override — see
   *  Viewer.tsx's own `fitFraction` doc. Undefined outside guided workflow's
   *  Review stage. */
