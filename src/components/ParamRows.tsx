...
interface Props {
  ...
  /**
   * Wave 2 (guided shell, `ui.workflow: "guided"`): shows an adjacent
   * "Import font…" affordance next to every `@font` param's FontSelect
   * control (see `showInlineFontImport` in `renderRow`) — reuses
   * FontImportActions, the same wiring AttentionItems.tsx's own "Import
   * font…" action and FileBar's font card use. Default false, so every
   * "tabs"-workflow caller (ParamForm's own default) is unaffected.
   */
  guided?: boolean;
}
...
function Control({
  ...
  switch (param.type) {
    ...
    case "enum": {
      // Full selected label as `title` on SelectValue's own span (item 3.2):
      // its trigger now truncates a long value with an ellipsis (ui/select.tsx),
      // so the hover tooltip is the only way to read it in full without opening
      // the dropdown.
      const selectedLabel = param.choices.find((c) => c.value === String(value))?.label ?? String(value);
      return (
        <Select value={String(value)} onValueChange={(v) => onChange(v)}>
          <SelectTrigger className="w-full" aria-label={label}>
            <SelectValue title={selectedLabel} />
          </SelectTrigger>
          <SelectContent>
            {param.choices.map((c) => (
...
// INLINE right after the label's own text (item 3.3 — see the renderRow call
// site below, which nests this inside the label `<span>` itself rather than
// beside it as a flex sibling). A flex sibling let the label wrap to two
// lines while the icon stayed pinned to the row's cross-axis, reading as
// detached "furniture" beside the paragraph instead of part of it; nesting
// it as the label span's trailing inline child instead makes it flow and
// wrap with the text like a normal word, landing right after the last one.
// The detail was previously reachable only through the hover-only `title`
// tooltip, which is invisible on touch devices.
function ParamHelp({ help, label }: { help: string; label: string }) {
  return (
    <Popover>
      <PopoverTrigger asChild>
        {/* 15px glyph, ~44px tap target (WCAG 2.2 target size): the negative
            margin absorbs the padding so the glyph's own inline footprint
            (and the label's line-height) stays close to the icon's visible
            size while the click/tap area itself is the full 44px box. */}
        <button
          type="button"
          ...
          className="param-help-trigger -m-[14.5px] inline-flex shrink-0 cursor-pointer items-center justify-center rounded-[4px] border-none bg-transparent p-[14.5px] align-middle leading-[0] text-muted-foreground hover:text-brand focus-visible:text-brand focus-visible:outline-offset-1 [&_svg]:h-[15px] [&_svg]:w-[15px]"
...
export const ParamRows = memo(function ParamRows({
  ...
  const renderRow = (p: Param) => {
    ...
    // Wave 2 (guided shell): every `@font` param gets an adjacent "Import
    // font…" affordance in guided mode, next to FontSelect — not just the
    // in-dropdown row FontSelect itself already offers (see its own doc),
    // which stays for every workflow but reads as a buried power-user path.
    // Suppressed exactly when the missing-font warning card below already
    // renders (`missingFontValue !== null`) — that card leads with its own
    // Import action, so a second one directly underneath would be redundant.
    const isFontParam = (p.type === "string" || p.type === "enum") && p.isFont;
    const showInlineFontImport = guided && isFontParam && !!installedFonts?.length && missingFontValue === null;
      ...
      <>
        <span className={`flex ${isToggle ? "items-center" : "items-baseline"} justify-between gap-2`}>
          {/* Label + optional info button together on the left so the
              right edge is free for the toggle / var-name code. The info
              button (item 3.3) nests INSIDE the label span, right after the
              label text, so it flows as ordinary inline content — wrapping
              onto the label's own last line like a trailing word — instead
              of sitting as a separate flex item beside a label that may wrap
              on its own (which stranded it floating beside the paragraph,
              not part of it). */}
        ...
        {!isToggle && control}
        {showInlineFontImport && (
          <FontImportActions
            className="mt-[0.1rem] flex gap-2"
            renderImport={(open) => (
              <Button
                type="button"
                variant="ghost"
                size="sm"
                className="param-font-import h-7 px-2 text-[0.78rem] text-muted-foreground"
                onClick={open}
              >
        ...
        {missingFontValue !== null && (
          // The contextual warning card (visual-alignment pass): a synthetic
          // single-item attention list, right under the control it's about —
          // AttentionItems.tsx's own doc explains why this omits both
          // "go to setting" (already at the setting) and "open Messages"
          // (not a notice item).
          <AttentionItems
            attention={[
              {
            ...
            className="font-missing mt-[0.1rem]"
          />
        )}
      </>
    );
    ...
    return (
      // Round-2 review fix (vertical rhythm): my-3 (12px, collapsing to a
      // ~12px gap between consecutive rows) -> my-(--space-2) (8px) —
      // "field groups gap ~8px" — and the row's own internal label-to-
      // control gap tightened from 0.35rem to --space-1 (4px).
      <div className="param my-(--space-2) flex flex-col gap-(--space-1)" key={p.name} data-param={p.name}>
        {p.filledBy ? (
  ...
  return (
    <div className="param-form">
      ...
      {groups.map(({ section, params }) => {
        if (sectionChrome === "flat") {
          ...
          return (
            <div className="param-group param-group--flat mb-3" key={section}>
              {showSectionHeadings && (
                <div className="font-display px-[0.2rem] py-[0.6rem] text-[0.92rem] font-semibold text-foreground">
                  {section}
                </div>
              )}
        ...
        return (
          // A visibly separated card (mockup target): --radius-card (bigger
          // than the old rounded-lg default) + --shadow-1 (a docked-surface-
          // appropriate whisper of lift, not the floating --shadow-2) on top
          // of the existing bg-background/50 wash, which is what actually
          // distinguishes the card's fill from the panel's own pure --panel
          // white behind it (a plain bg-card here would be invisible against
          // that panel — --bg and --panel deliberately differ for exactly
          // this reason). Comfortable --space-4 padding, a stronger title
          // row (larger + bolder) below.
          <details
            className="param-group mb-(--space-4) rounded-(--radius-card) border bg-background/50 px-(--space-4) shadow-(--shadow-1) open:pb-(--space-3)"
            key={section}
            open={q ? true : isOpen}
            onToggle={(e) => {
          ...
          >
            <summary className="font-display flex cursor-pointer select-none list-none items-center px-[0.2rem] py-(--space-3) text-[0.95rem] font-bold text-foreground hover:text-brand focus-visible:rounded-[4px] focus-visible:text-brand">
              {section}
            </summary>
            {params.map(renderRow)}
          </details>
