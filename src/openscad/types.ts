...
// The worker protocol itself lives in protocol.ts and is RE-EXPORTED here, not
// re-declared: protocol.ts is inside renderHash's hashed closure and this file
// is deliberately not (see CLAUDE.md), but both ends of the postMessage wire
// have to be typed from one declaration or tsc cannot see them diverge — the
// boundary is MessageEvent.data, which type-checks against nothing.
export type {
  ParamValue,
  ModelFormat,
  RenderRequest,
...
export interface Schema {
  ...
  /**
   * Config for the Files dialog, which MANAGES imported files; it is not a
   * generic import button. Importing is contextual, at the control that needs
   * the file — see `FileImport`'s own doc, which is the current one. Null/absent
   * means no Files action at all. All copy is config-driven so this stays
   * project-agnostic.
   */
