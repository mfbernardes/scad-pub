...
export class OpenSCADRunner {
  ...
  // M-progress: forwards the worker's bootstrap-download progress messages
  // (see worker.ts's cachedBufferWithProgress). Suppressed once `readyFired`
  // is set (below) — a late/stale message from a respawned worker (spawn()
  // resets bootstrap, which typically hits Cache Storage and posts nothing,
  // but isn't guaranteed to) must never resurrect the pre-ready loading UI
