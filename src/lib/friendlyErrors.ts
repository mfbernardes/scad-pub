...
/** Cap on the number of technical detail lines kept. */
 ...
 * order.
 */
export function friendlyRenderError(result: RenderResult | null): FriendlyErrorInfo | null {
  if (!result || result.ok) return null;
  const technical = technicalTail(result.log, DEFAULT_MAX_TECHNICAL);
