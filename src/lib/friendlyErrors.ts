// friendlyErrors.ts — maps a failed RenderResult onto the friendly copy shown
// by FriendlyError.tsx (the Notices tab's failure card) and used for the
// render-failed toast title (useRenderPipeline.ts). Deliberately NOT a React
// component or hook — a pure function over the render outcome, kept
// dependency-free (besides i18n) so tests/friendlyErrors.test.mjs can drive
...
//     existing failure toast text exactly (same key) so the toast and the
//     Notices card never say two different things for the same outcome.
//
...
export interface FriendlyErrorInfo {
   ...
   *  lines — the "Show technical details" disclosure's content. Never the
   *  whole log. */
  technical: string[];
}
...
export function friendlyRenderError(
  ...
  if (result.fatal) return { title: t("failure.engineFailed"), body: null, technical };
  ...
  if (firstAssert !== null)
    return { title: t("failure.title"), body: assertMessage(firstAssert) ?? firstAssert, technical };

  return { title: t("toast.renderFailed"), body: null, technical };
}
