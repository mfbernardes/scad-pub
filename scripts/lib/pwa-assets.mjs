...
export function generatePwaAssets({
  ...
  if (config.icon) {
    ...
    iconSvg =
      ...
      `</svg>\n`;
    write("icon.svg", iconSvg, "default icon.svg");
  }
  ...
  if (rasterize) {
    const beforeFailure = [...written]; // for cleanup below, on failure
    try {
      write("icon-192.png", rasterize(iconSvg, 192), "icon-192.png");
      iconFiles.push("icon-192.png");
      write("icon-512.png", rasterize(iconSvg, 512), "icon-512.png");
      iconFiles.push("icon-512.png");
      write("icon-512-maskable.png", rasterize(maskableSvg, 512), "icon-512-maskable.png");
      iconFiles.push("icon-512-maskable.png");
      write("icon-180.png", rasterize(iconSvg, 180), "icon-180.png");
      iconFiles.push("icon-180.png");
    } catch (err) {
      // Don't leave a partial icon batch (e.g. icon-192.png wrote fine, then
      // icon-512.png's source SVG failed) sitting on disk unreferenced by
      // anything — nothing will reconcile it later since generate() throws
      // before recording this run's owned-file manifest.
