...
// Plain Levenshtein edit distance between two strings — the same tiny DP used
// for the `strings` "did you mean" suggestion below. Kept local (no npm
// dependency) since it's only ever called on short dot-namespaced key names.
...
// module stays free of file I/O. Fails the build with a clear message (a "did
// you mean" suggestion when the bad key is a plausible typo of a real one)
// rather than silently accepting a key `t()` will never resolve. Returns {}
// when unset.
