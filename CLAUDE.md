...
`App.tsx` orchestrates debounced auto-render, the "heavy render" brake, export, presets, fonts, URL state, theme, and the PWA install/offline notices. Renders slower than `HEAVY_RENDER_MS` (about 6 s) auto-pause live updates; designs flagged `heavy` start in manual mode. `AppShell.tsx` owns the responsive layout — desktop docked `ParamPanel` or mobile `BottomSheet` — plus the panel-facing derived state (attention/readiness/checklist) computed from props `App.tsx` hands it; it is not a pure view extraction (PR21b is expected to pull that derivation back out). Most state still lives in `App.tsx`. Only the active layout mounts a `Viewer` (`useIsMobile`), which is lazy-loaded to keep three.js out of the initial JS chunk.
