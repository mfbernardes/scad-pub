...
- **OpenSCAD annotations** are parsed by `gen-schema` and invisible to OpenSCAD/desktop Customizer. The supported annotations are `// @showIf <expr>`, `// @collapsed`, `// @font`, `// @info [Label | unit]`, `// @svg [layers=<param>]`, `// @filledBy <param>`, and file-level `// @description` / `// @icon`. See [docs/annotations.md](docs/annotations.md) for behavior and examples.
