...
`src/generated/designs.json`, `src/generated/locales.json`, `public/scad/`, `public/wasm/`, the generated PWA assets (`public/manifest.webmanifest`, `public/icon.svg`, `public/icon-*.png`, `public/apple-splash-*.png`, `public/precache-manifest.json`), and `public/fonts/fonts.conf` are **gitignored / generated**. Never edit them by hand; change the config or sources and re-run. The bundled `.ttf` files under `public/fonts/` are tracked, and so is the hand-written `public/sw.js`.
...
`App.tsx` orchestrates debounced auto-render, the "heavy render" brake, export, presets, fonts, URL state, theme, and the PWA install/offline notices. Renders slower than `HEAVY_RENDER_MS` (about 6 s) auto-pause live updates; designs flagged `heavy` start in manual mode. `AppShell.tsx` owns the responsive layout — desktop docked `ParamPanel` or mobile `BottomSheet` — as a pure view extraction: it calls `usePanelDerivedState` (`src/lib/usePanelDerivedState.ts`) to turn `App.tsx`'s render/design state into the panel-facing business state (attention/readiness/the getting-started checklist/the hidden-advanced-diff/the friendly render-failure summary/whether QuickStart is active), then hands that bundle to a single `PanelDataProvider` mount (`src/lib/panelData.ts`, see below) instead of computing it inline or threading it through `ParamPanel`/`SheetTabs` prop-by-prop. All state stays in `App.tsx`. Only the active layout mounts a `Viewer` (`useIsMobile`), which is lazy-loaded to keep three.js out of the initial JS chunk.
...
`PanelDataContext` (`src/lib/panelData.ts`) is `AppActions`' sibling for panel **data**: the ~25 fields `CustomizeTab`/`GettingStarted` read identically whether mounted by the desktop `ParamPanel` or the mobile `SheetTabs` (design/values/baseline/attention/readiness/the checklist/…), which those two used to each declare and thread through by hand. `AppShell` mounts one `PanelDataProvider` above both layouts; `CustomizeTab`/`GettingStarted` read the bundle via `usePanelData()`, and `ParamPanel`/`SheetTabs` shrink to their genuinely layout-specific props (tab/search wiring, `sheetDetent`, panel sizing, the presets/file-import props `PresetPicker`/`FileBar` consume directly). Unlike `AppActions`, this context's value is a **plain, freshly-memoized-per-render** object, not a stable-ref proxy: it carries live state that must actually propagate (values, attention, readiness, …), so hiding updates behind a never-changing identity would freeze every consumer's view of it — ordinary React context semantics apply here instead.
...
- **User-facing strings go through `src/lib/i18n.ts`'s `t()`/`tn()`; add keys to BOTH locale bundles** (`src/locales/en.json` and `de.json`, same dot-namespace conventions — `tabs.*`, `presets.*`, `files.*`, `console.*`, `dialog.*`, `viewer.*`, etc.). This covers visible text, `aria-label`/`title`/`placeholder`, and sr-only text alike. Exceptions: `src/lib/renderStatus.ts` (harness-load-bearing, left hand-written on purpose), config-driven deployment content (help/popup/design metadata/preset names/notices labels), `THIRD-PARTY-LICENSES`/license body text in `src/lib/licenses.ts`, and messages that only ever reach `console.warn`/`console.error`. `src/locales/en.json`/`de.json` stay the SOURCE of truth (`tests/i18n.test.mjs` enforces en/de key parity against them directly), but `i18n.ts` itself imports the GENERATED `src/generated/locales.json`: `gen-schema.mjs` resolves the config's `lang` at build time and emits only the bundle(s) actually needed (English, plus the resolved locale's own bundle when it isn't English) instead of shipping every bundled locale's text in every build. `tests/i18nCoverage.test.mjs` catches a key that's gone dead (declared but no longer referenced anywhere in `src/`).
...
- **OpenSCAD annotations** are parsed by `gen-schema` and invisible to OpenSCAD/desktop Customizer. The supported annotations are `// @showIf <expr>`, `// @collapsed`, `// @font`, `// @info [Label | unit]`, `// @svg [layers=<param>]`, `// @filledBy <param>`, `// @advanced`, `// @essential`, `// @step <id> [| <label>]`, and file-level `// @description` / `// @icon` / `// @doc`. See [docs/annotations.md](docs/annotations.md) for behavior and examples.
