...
- **`assets`**: files, directories, or glob patterns (`*`/`?`/`**`) to bundle. A named file is copied as-is; a directory contributes only the `.scad` files under it (recursively) — use a glob (`lib/**`, `icons/*.svg`) or name the file directly to bundle non-`.scad` content. If omitted, `gen-schema` follows each design’s `use`/`include` graph
...
#### Translating a design's own doc

A design’s `// @doc` (see [annotations.md](annotations.md#design-metadata--description--icon--image--doc)) is a whole Markdown file, not a JSON field, so it has its own sidecar shape — but unlike `.strings.<tag>.json`, that sidecar sits beside the **doc file itself**, not the `.scad`: a shipped locale tag inserted before the doc’s own extension, `<docbase>.<tag><ext>`. A design whose doc lives beside its `.scad` and is named the ordinary way (`// @doc nameplate.md`) gets a German translation at `nameplate.de.md`, right beside it; a design whose doc lives elsewhere (`// @doc docs/guides/nameplate.md`) gets its translation at `docs/guides/nameplate.de.md` — wherever `@doc` actually points, not wherever the `.scad` sits. This mirrors both the `.strings.<tag>.json` convention (a translation sits next to the file it translates) and how per-locale help Markdown is conventionally named (`<name>.<tag>.md` beside `<name>.md`).

The discovery mechanics mirror `.strings.<tag>.json` above, just rooted at the doc’s own directory and basename instead of the design’s: a per-tag probe, a case-insensitive directory scan that catches a wrongly-cased or unshipped tag, an orphan warning for one left behind when a doc is renamed (checked against every doc currently live in that directory, not just this design’s). It is only legal for a design that actually has a `// @doc`: a doc translation with no base doc to translate fails the build, the same as translating a `description` a design never set. A leftover translation using the retired `<design>.doc.<tag>.md` naming (beside the `.scad`, regardless of where the doc itself lives) fails the build too, naming the file and exactly where it now belongs.

Like the base doc itself, a translation is copied to a served URL (`<id>-doc.<tag>.md`) and fetched by the browser on demand — never inlined into `designs.json`, never folded into `renderHash`. `DesignDocModal` fetches the active locale’s translation when one exists, falling back to the design’s own (authored-language) doc for any locale that has none.
...
Omit `notices`, or set it to `[]`, and no marker categories are recognised. Design echoes appear only in the raw log. The bundled example config (`scadpub.config.json`) opts in with `alert` and `note` categories. The example `diagnostics` design echoes them from its own switches, alongside a warning and an assert failure, so you can see each badge appear and disappear; the other example designs are deliberately silent at their defaults.
