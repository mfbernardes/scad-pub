...
`ui.workflow` picks the overall shape of the Customize-panel internals, the terminal Review stage, and the mobile sheet's Review detent. It is completely independent of `ui.experience`'s guided/standard client toggle — a visitor can be in guided *experience* under either workflow; `ui.workflow` decides what that guided experience's QuickStart step navigation actually looks like once it's showing (guided experience + essentials settings view + a stepped design — see [`quickStart`](#ui-options) above). Never affects geometry, so it never invalidates the render cache. The export dock itself (Download + Share, no menus — see "Direct export, no menu" below) is no longer part of what this flag changes: it's identical in every workflow. The standing Essential/All `SettingsViewToggle` and its standing Live-preview switch are likewise no longer workflow-specific: both are replaced by the same per-stage "Show advanced settings" model (see below) whenever QuickStart shows, in either workflow. The standing toggle only remains for a design where QuickStart *doesn't* show — a stepless design, or standard experience.

- **`"tabs"`** (the default): today's behavior, byte-for-byte unchanged, except the export dock (see "Direct export, no menu" below, which now applies in every workflow) and advanced settings, which are now harmonized with `"guided"` (see below) wherever QuickStart shows: the standing Essential/All `SettingsViewToggle` is gone in favor of the same per-stage "Show advanced settings" model. QuickStart's desktop panel stacks every visible step in one scrollable flow, each with its own advanced toggle; the terminal Review stage carries its own standing Export button.
