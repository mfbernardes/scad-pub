...
`ui.workflow` picks the overall shape of the Customize-panel internals, the terminal Review stage, the export dock, and the mobile sheet's Review detent. It is completely independent of `ui.experience`'s guided/standard client toggle — a visitor can be in guided *experience* under either workflow; `ui.workflow` decides what that guided experience's QuickStart step navigation actually looks like once it's showing (guided experience + essentials settings view + a stepped design — see [`quickStart`](#ui-options) above). Never affects geometry, so it never invalidates the render cache.

- **`"tabs"`** (the default): today's behavior, byte-for-byte unchanged. QuickStart's desktop panel stacks every visible step in one scrollable flow; the standing Essential/All `SettingsViewToggle` sits above the form; the terminal Review stage carries its own standing Export button; the export dock is a split Download button (a small "▾" trigger holding the format note) plus a Share/More row (Save image, Copy link).
