...
```jsonc
{
  ...
  "viewerControls": true,         // overlay zoom/reset buttons on the preview; default true
...
- **`viewerControls`** — boolean, default `true`. Shows the map-style overlay buttons on the 3D preview (zoom in, zoom out, reset view). Set to `false` to hide them at build time; orbit/zoom by mouse or touch still works.
