...
A full review of the runtime app (`src/`), the build pipeline (`scripts/`,
`vite.config.ts`, `public/sw.js`), and the test/CI setup, as of `0cf00d1`
(axe-core AA sweep). Findings are ordered by severity within each section;
every claim carries a `file:line` reference against that commit.

## Verdict

The codebase is in unusually good shape for a client-side WASM app. The
client/worker split is strict, the two-tier render cache degrades gracefully,
the stable-identity `AppActions` context genuinely avoids both prop-drilling
and re-render churn, and hygiene is near-perfect: zero `as any` /
`@ts-ignore` / `@ts-expect-error` in `src/**`, no TODO/FIXME markers, three
justified `eslint-disable`s total. The findings below are the residue, not a
rescue list. The highest-value fixes are the two HTML-injection gaps in the
config-to-chrome path (which contradict a documented guarantee), the silent
hard-failure path in `doRender`, and the stale visual-regression mask.
...
### 3. A thrown render failure is invisible to the user
...
(`App.tsx:177-181`). The UI just stops spinning with a stale model on
...
`AppShell` — but every new feature lands here. Three extractions fall out
...
assertion is easy to localize. Splitting `generate()` at its own banner
...
- Playwright Chromium is reinstalled on every run (`ci.yml:24-25` caches npm
  only) — cacheable for a minute or two per build.
...
- **Latest-wins render cancellation** with terminate-and-respawn, staleness
