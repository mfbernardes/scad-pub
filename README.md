...
- **Presets** — a picker with **Ready-made** (read-only, shipped per design) and **Saved by you** (kept in the browser) sections; save the current parameters as a named preset. Bundled presets use OpenSCAD's `parameterSets` JSON format, so they round-trip with the desktop Customizer.
- **Responsive UI** — full-bleed 3D canvas with a docked, resizable parameter panel on desktop and a persistent, detented bottom sheet on mobile. The theme switch and render-status indicator sit in the top bar of both layouts, so nothing is desktop-only. Built on [shadcn/ui](https://ui.shadcn.com/) (Radix primitives + Tailwind v4) over AA-tuned colour tokens, with its own design language — cool slate neutrals, an electric-blue accent, a brand-blue model material, clean rounded-rectangle controls, and the Atkinson Hyperlegible Next display face. Every token (colours, radii, fonts) is overridable per deployment via the config, so a consumer project can re-skin it completely.
...
- **Notices & log** — OpenSCAD `echo` notices, warnings and `assert` failures are parsed into a **Messages** console, opened from the bell in the top bar — it rings and shows a count badge while notices are pending (and auto-opens the first time a render surfaces a notice or assert). Its Notices tab lists them with per-category coloured count badges; the raw log sits one tab over. See [docs/config.md](docs/config.md#notice-badges-notices).
...
- **Live preview with brake** — re-renders after a debounce. Designs flagged `heavy` start in manual mode. Any render slower than ~6 s auto-pauses live updates; an "update now" banner on the preview takes over.
